...
export default {
  ...
  data() {
    return {
      ...
      invoice_statuses: [],
      currency: '',
      columns: ['item.name', 'invoice.invoice_number', 'invoice.customer.user.name', 'quantity', 'quantity_supplied', 'balance', 'rate', 'amount', 'created_at'],
  ...
  methods: {
    ...
    getInvoices() {
      ...
      outboundReport.list(param)
        .then(response => {
          ...
          app.table_title = 'Unsupplied Invoices  in ' + app.warehouses[param.warehouse_index].name + ' from: ' + app.form.from + ' to: ' + app.form.to;
          loader.hide();
        })
        .catch(error => {
          loader.hide();
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const tHeader = ['Product', 'Invoice No.', 'Customer', 'Quantity', 'Quantity Supplied', 'Unsupplied', 'Rate', 'Amount', 'Created At'];
