        ...
        <section class="invoice">
            ...
            <div class="col-xs-12 page-header">
              ...
              <span>
                ...
                <div class="pull-right no-print">
                  <a @click="doPrint();"><i class="el-icon-printer" /> Print Invoice</a>
                </div>
              </span>
            </div>
            <!-- /.col -->
          ...
          <div class="row invoice-info">
            <div class="col-sm-4 invoice-col">
              ...
              <address>
                <label>{{ invoice.customer.user.name.toUpperCase() }}</label><br>
                {{ (invoice.customer.type) ? invoice.customer.type.name.toUpperCase() : '' }}<br>
                Phone: {{ invoice.customer.user.phone }}<br>
                Email: {{ invoice.customer.user.email }}<br>
            ...
            <div class="col-sm-4 invoice-col">
              ...
              <address>
                <strong>{{ invoice.warehouse.name.toUpperCase() }}</strong><br>
                {{ invoice.warehouse.address }}<br>
              </address>
            </div>
            <!-- /.col -->
            <div class="col-sm-4 invoice-col">
              <label>Invoice No.: {{ invoice.invoice_number }}</label><br>
              <label>Date:</label> {{ moment(invoice.created_at).format('MMMM Do YYYY, h:mm:ss a') }}<br>
            </div>
            <!-- /.col -->
          </div>
          <!-- /.row -->
                ...
                <tbody>
                  <tr v-for="(invoice_item, index) in invoice.invoice_items" :key="index">
                    ...
                    <td>
                      ...
                      <br><small>({{ invoice_item.no_of_cartons }} CTN)</small>
                    </td>
                  ...
                  <tr>
                    ...
                    <td align="right"><label style="color: green">{{ currency + Number(invoice.amount).toLocaleString() }}</label></td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <el-tab-pane label="Invoice History" name="second">
        <div class="row">
          <div class="col-xs-12">
            ...
            <div class="pull-right no-print">
              <a class="btn btn-default" @click="doPrint();"><i class="el-icon-printer" /> Print History</a>
              <a :loading="downloadLoading" class="btn btn-info" @click="handleDownload"><i class="el-icon-download" /> Export Excel</a>
            </div>
          </div>
        </div>
        <div class="block">
          <el-timeline>
            <el-timeline-item v-for="(history, index) in invoice.histories" :key="index" :timestamp="moment(history.created_at).format('MMMM Do YYYY, h:mm:ss a')" placement="top">
              <el-card>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const multiHeader = [['Invoice History for Invoice No.: ' + this.invoice.invoice_number, '', '']];
