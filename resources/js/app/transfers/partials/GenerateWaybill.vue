              ...
              <el-col :xs="24" :sm="12" :md="12">
                ...
                >
                  <el-option
                    ...
                    :key="invoice_index"
                    :value="invoice_index"
                    :label="(invoice.request_warehouse) ? invoice.request_warehouse.name + '[ '+ invoice.request_number + '] ' : invoice.request_number"
                  />
                </el-select>
              </el-col>
  ...
  methods: {
    ...
    fetchUndeliveredInvoices(index) {
      ...
      unDeliveredInvoices.list(form).then((response) => {
        app.invoices = response.transfer_requests;
    ...
    displayInvoiceitems() {
      ...
      for (let index = 0; index < selected_invoice.length; index++) {
        ...
        invoice_items.push(...app.invoices[element].transfer_request_items);
