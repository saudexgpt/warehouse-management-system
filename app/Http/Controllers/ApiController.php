...
class ApiController extends Controller
{
    ...
    public function productInStock(Request $request)
    {
        ...
        $items_in_stock_query->whereRaw('quantity - supplied - expired > 0')
            ...
            ->orderBy('expiry_date')
            ->select('warehouses.name as warehouse', 'items.id as product_id', 'items.name as product', 'items.basic_unit as uom', 'quantity_per_carton', 'categories.name as product_type', 'categories.group_name as product_category', 'batch_no', 'goods_received_note as grn', \DB::raw('SUM(quantity) as quantity_in'), \DB::raw('(SUM(supplied) + SUM(expired)) as quantity_out'), \DB::raw('(SUM(quantity) - SUM(supplied) - SUM(expired)) as total_balance'), \DB::raw('(SUM(quantity) - SUM(supplied) - SUM(expired))/quantity_per_carton as total_balance_in_carton'), 'expiry_date', 'item_stock_sub_batches.created_at');
    ...
    public function sendRepStock(Request $request)
    {
        ...
        if (isset($request->date_from, $request->date_to) && $request->date_from != '' && $request->date_to != '') {
            ...
            $customer_ids = Customer::join('users', 'customers.user_id', 'users.id')
                // ->where('customers.type', 'reps')
                ->where('users.email', $email)
                // ->whereIn('customers.code', $customer_codes_array)
                ->pluck('customers.id');
